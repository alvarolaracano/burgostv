...
public class VideoActivity extends Activity implements OnCompletionListener, SeekBar.OnSeekBarChangeListener{

    //private static final String path ="http://www.burgostv.es/noticias/2012/julio/25-07-12_2-SD.mp4";
	...
	public void onCreate(Bundle icicle) {
	        ...
	        video.setOnTouchListener(new View.OnTouchListener() {
				...
				public boolean onTouch(View v, MotionEvent event) {
					 ...
					 if(video.isPlaying()){
						 video.pause();
						 //Toast.makeText(v.getContext(), "pausa", Toast.LENGTH_LONG).show();
						 //Mostrar el boton de play.
						 play.setVisibility(View.VISIBLE);
						 updateProgressBar();
					 }else{
						 video.start();
						 //Toast.makeText(v.getContext(), "play", Toast.LENGTH_LONG).show();
	...
	public void onStopTrackingTouch(SeekBar seekBar) {
		...
		int posicionActual = utils.progressToTimer(seekBar.getProgress(), duracionTotal);
		
		int posicionBuffer = seek.getSecondaryProgress();
		
		//Si la posicion del seek es > del 15 % del buffer, mantenemos donde estaba.
		
		int posicion = (posicionActual*100)/duracionTotal;
		
		//Toast.makeText(getApplicationContext(), "posicion: "+posicion+" => buffer "+posicionBuffer, Toast.LENGTH_LONG).show();
		
		//posicionbuffer-15 para un rango.
		if(posicion < posicionBuffer){
			// Movernos a la posicionActual.
			
			//Toast.makeText(getApplicationContext(), "SI mueve posicion", Toast.LENGTH_LONG).show();
			video.seekTo(posicionActual);
			
		}else{
			//Toast.makeText(getApplicationContext(), "NO mueve posicion", Toast.LENGTH_LONG).show();
		}
	...
	private Runnable mUpdateTimeTask = new Runnable() {
		   public void run() {
			   ...
			   total.setText(""+utils.milliSecondsToTimer(duracionTotal));
			   
			   // Mostar el tiempo actual. Si no tiene un 00, añadir un espacio.
			   if(utils.milliSecondsToTimer(duracionActual).toString().length() == 4){
				   actual.setText(" "+utils.milliSecondsToTimer(duracionActual)+" /");
			   }else{
				   actual.setText(""+utils.milliSecondsToTimer(duracionActual)+" /");
			   }
			   
			   // Updating progress bar
			   int progress = (int)(utils.getProgressPercentage(duracionActual, duracionTotal));
